...
from genre import genres,name
import csv

#25M数据
VecFile ='../models/item2vec-Anime-model.vec'
VecFile2='../models/Graph-item2vec-Anime-model.emb'
...
def topk_like(cur_movie_id, vector,k=5, print_log=False)->int:
    ...
    for movie_id, vec in vector.items():
        ...
        if len(like_candidates) < k or sim > min_heap.get_min() and movie_id in genres.keys():
            min_heap.add(sim)
    ...
    if print_log:
        for t in sorted(like_candidates, reverse=True, key=lambda _:_[1])[:k]:
            logger.info('         [%s] %f  %s' % (t[0], t[1],genres[t[0]]))
            for genre in list(genres[cur_movie_id].strip().split(',')):
...
if __name__ == '__main__':
    ...
    for k in range(1,21):
        ...
        for i in range(0, 1000):
            ...
            if temp in vectors2.keys() and temp in genres.keys() and temp in name.keys():
                seed.append(temp)
        movie_ids = seed[:num]
